...
class QAsyncioTask(futures.QAsyncioFuture):
    ...
    def __init__(
        ...
        *,
        ...
        self._state = futures.QAsyncioFuture._STATE_PENDING
        ...
        self._must_cancel = False  # compatibility with asyncio.Task API (used by anyio)
    ...
    def _fut_waiter(self):
        """Compatibility with asyncio.Task internal API (used by anyio)."""
        return self._future_to_await

    @_fut_waiter.setter
    def _fut_waiter(self, value):
        self._future_to_await = value

    def __repr__(self) -> str:
        state: str = "Unknown"
        if self._state == futures.QAsyncioFuture._STATE_PENDING:
            state = "Pending"
        elif self._state == futures.QAsyncioFuture._STATE_DONE_WITH_RESULT:
            state = "Done"
        elif self._state == futures.QAsyncioFuture._STATE_DONE_WITH_EXCEPTION:
            state = f"Done with exception ({repr(self._exception)})"
        elif self._state == futures.QAsyncioFuture._STATE_CANCELLED:
            state = "Cancelled"
    ...
    def _step(
        self, exception_or_future: BaseException | futures.QAsyncioFuture | None = None
    ) -> None:
        """
        ...
        # Inline isfuture check: avoid function call overhead
        if (
            exception_or_future is not None
            and not isinstance(exception_or_future, BaseException)
            and getattr(exception_or_future, "_asyncio_future_blocking", None)
            is not None
        ):
            try:
        ...
        _enter_task = asyncio._enter_task
        _leave_task = asyncio._leave_task
        _loop = self._loop

        try:
            _enter_task(_loop, self)  # type: ignore[arg-type]
            ...
            self._state = futures.QAsyncioFuture._STATE_DONE_WITH_RESULT
            self._result = e.value
        except (
            concurrent.futures.CancelledError,
            asyncio.exceptions.CancelledError,
        ) as e:
            self._state = futures.QAsyncioFuture._STATE_CANCELLED
            self._exception = e
        except BaseException as e:
            self._state = futures.QAsyncioFuture._STATE_DONE_WITH_EXCEPTION
            self._exception = e
            self._tb = traceback.format_exc()
        else:
            # Inline isfuture check for result
            if (
                result is not None
                and getattr(result, "_asyncio_future_blocking", None) is not None
            ):
                ...
                _loop.call_soon(self._step, context=self._context)
            else:
                # This is not supposed to happen.
                exception = RuntimeError(f"Bad task result: {result}")
                _loop.call_soon(self._step, exception, context=self._context)
        finally:
            _leave_task(_loop, self)  # type: ignore[arg-type]
                ...
                _loop.call_exception_handler(
                    {
                        ...
                        "future": (
                            exception_or_future
                            if exception_or_future is not None
                            and not isinstance(exception_or_future, BaseException)
                            and getattr(
                                exception_or_future, "_asyncio_future_blocking", None
                            )
                            is not None
                            else None
                        ),
