...
class QAsyncioEventLoop(asyncio.BaseEventLoop, QObject):
    ...
    def __init__(self, application: QCoreApplication, quit_qapp: bool = True) -> None:
        ...
        # fd -> (QSocketNotifier, callback, args) for async socket I/O
        self._readers: dict[int, tuple[QSocketNotifier, Callable, tuple]] = {}
        self._writers: dict[int, tuple[QSocketNotifier, Callable, tuple]] = {}

        # fd -> transport, needed by _SelectorSocketTransport
    ...
    def _call_soon_impl(
        ...
        *args: Unpack[_Ts],
        ...
        return QAsyncioHandle(
            callback, args, self, context, is_threadsafe=is_threadsafe
        )
    ...
    async def create_server(
        ...
        *,
        ...
        raise NotImplementedError(
            "QAsyncioEventLoop.create_server() is not implemented yet"
        )
    ...
    async def create_unix_server(
        ...
        *,
        ...
        raise NotImplementedError(
            "QAsyncioEventLoop.create_unix_server() is not implemented yet"
        )
    ...
    def _add_reader(self, fd, callback, *args):
        ...
        if fd in self._readers:
            self._remove_reader(fd)
        notifier = QSocketNotifier(QSocketNotifier.Type.Read, self)
        notifier.setSocket(fd)
        notifier.activated.connect(
            lambda *_a, _fd=fd: self._notifier_cb(_fd, self._readers)
        )
        notifier.setEnabled(True)
        self._readers[fd] = (notifier, callback, args)
    ...
    def _add_writer(self, fd, callback, *args):
        ...
        if fd in self._writers:
            self._remove_writer(fd)
        notifier = QSocketNotifier(QSocketNotifier.Type.Write, self)
        notifier.setSocket(fd)
        notifier.activated.connect(
            lambda *_a, _fd=fd: self._notifier_cb(_fd, self._writers)
        )
        notifier.setEnabled(True)
        self._writers[fd] = (notifier, callback, args)
    ...
    def _sock_recv(self, fut, registered_fd, sock, n):
        ...
        try:
            ...
            self._add_reader(fd, self._sock_recv, fut, True, sock, n)
    ...
    def _sock_recv_into(self, fut, registered_fd, sock, buf):
        ...
        try:
            ...
            self._add_reader(fd, self._sock_recv_into, fut, True, sock, buf)
        except (SystemExit, KeyboardInterrupt):
            raise
        except BaseException as exc:
            fut.set_exception(exc)
    ...
    async def sock_sendall(self, sock, data):
        ...
        fut = self.create_future()
        fd = sock.fileno()
        self._sock_sendall(fut, fd, sock, memoryview(data))
        return await fut

    def _sock_sendall(self, fut, fd, sock, data):
        if fut.cancelled():
            return
        try:
            n = sock.send(data)
        ...
        if n == len(data):
            ...
            if n:
                data = data[n:]
            self._add_writer(fd, self._sock_sendall, fut, fd, sock, data)
    ...
    def _sock_connect(self, fut, sock, address):
        ...
        try:
            ...
            fut.add_done_callback(lambda f: self._remove_writer(fd))
            self._add_writer(fd, self._sock_connect_cb, fut, sock, address)
        except (SystemExit, KeyboardInterrupt):
            raise
        except BaseException as exc:
            fut.set_exception(exc)
    ...
    def _sock_accept(self, fut, registered_fd, sock):
        fd = sock.fileno()
        if registered_fd:
            self._remove_reader(fd)
        if fut.cancelled():
        ...
        try:
            ...
            self._add_reader(fd, self._sock_accept, fut, True, sock)
        except (SystemExit, KeyboardInterrupt):
            raise
        except BaseException as exc:
            fut.set_exception(exc)
    ...
    # DNS — inherited from BaseEventLoop (uses run_in_executor)

    # Working with pipes

    async def connect_read_pipe(self, protocol_factory, pipe):
        raise NotImplementedError(
            "QAsyncioEventLoop.connect_read_pipe() is not implemented yet"
        )

    async def connect_write_pipe(self, protocol_factory, pipe):
        raise NotImplementedError(
            "QAsyncioEventLoop.connect_write_pipe() is not implemented yet"
        )
    ...
    def add_signal_handler(self, sig, callback, *args):
        raise NotImplementedError(
            "QAsyncioEventLoop.add_signal_handler() is not implemented yet"
        )
    ...
    def default_exception_handler(self, context: dict[str, Any]) -> None:
        ...
        if context["message"]:
            print(
                f"{context['message']} from task {context['task']._name},"
    ...
    async def subprocess_exec(
        ...
        **kwargs,
        ...
        raise NotImplementedError(
            "QAsyncioEventLoop.subprocess_exec() is not implemented yet"
        )
    ...
    async def subprocess_shell(
        ...
        **kwargs,
        ...
        raise NotImplementedError(
            "QAsyncioEventLoop.subprocess_shell() is not implemented yet"
        )
...
class QAsyncioHandle:
    ...
    # Use integer constants instead of enum for faster comparison
    _PENDING = 0
    _CANCELLED = 1
    _DONE = 2

    # Keep the enum class for backward compatibility but don't use it in hot paths
    ...
    def _schedule_event(self, timeout: int) -> None:
        ...
        loop = self._loop
        if not loop._closed and not loop._quit_from_outside:
            if self._is_threadsafe:
                ...
                QTimer.singleShot(timeout, loop, self._cb)
            else:
                QTimer.singleShot(timeout, self._cb)
    ...
    def cancel(self) -> None:
        if self._state == QAsyncioHandle._PENDING:
            # The old timer that was created in __init__ will still trigger but
            # _cb won't do anything, therefore the callback is effectively
            # cancelled.
            self._state = QAsyncioHandle._CANCELLED

    def cancelled(self) -> bool:
        return self._state == QAsyncioHandle._CANCELLED
...
class QAsyncioTimerHandle(QAsyncioHandle, asyncio.TimerHandle):
    def __init__(
        ...
        # Inline the base init to avoid double _schedule_event call.
        # QAsyncioHandle.__init__ would schedule with timeout=0, then we'd
        # schedule again with the real timeout — wasteful. Instead, do it once.
        self._callback = callback
        self._cb_args = args
        self._loop = loop
        self._context = context
        ...
        self._timeout = round(max(self._when - loop.time(), 0) * 1000)

        self._schedule_event(self._timeout)
